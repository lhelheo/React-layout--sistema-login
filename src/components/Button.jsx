    ...
    <ul className={`${buttonStyle} group`} onClick={onClick}>
      <li className="flex w-full justify-between">
        <div className='flex items-center'>
          {Icon1 && <Icon1 strokeWidth={2} className={`${iconAndTextClass} ${hoverClass}`}/>}
          <p className={`${iconAndTextClass} ${hoverClass} mobile:hidden`}>{label}</p>
        </div>
        {Icon2 && <Icon2 strokeWidth={2.6} className={`${iconAndTextClass} ${hoverClass} mobile:hidden flex items-center h-8`} />}
      </li>
    </ul>
  );
}
